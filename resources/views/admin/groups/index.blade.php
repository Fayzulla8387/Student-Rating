            ...
            <div class="card">
                ...
                <div class="card-body">
                    <table class="table table-hover">
                        ...
                        <tbody>
                        @foreach($groups as $firm)
                            <tr>
                                <td>{{$loop->index +1}}</td>
                                <td>
                                    <a href="{{ route('students.index', ['group_id' => $firm->id]) }}" class="page-link">
                                        {{$firm->name}}
                                    </a>
                                </td>
                                <td class="d-flex">
                                    ...
                                    <form action="{{route('groups.destroy', $firm->id)}}" method="post">
                                        @method('DELETE')
                                        @csrf
